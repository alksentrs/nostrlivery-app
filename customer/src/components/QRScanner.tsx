...
export const QRScanner: React.FC<QRScannerProps> = ({ onScan, onClose }) => {
  ...
  const handleBarCodeScanned = ({ data }: { type: string; data: string }) => {
    ...
    if (data.startsWith("npub1")) {
      ...
      Alert.alert(
        "Invalid QR Code",
        "This QR code does not contain a valid company NPUB. Please scan a company's QR code.",
        [
          {
            text: "Try Again",
      ...
      <View style={styles.container}>
        <View style={styles.permissionContainer}>
          ...
          <Text style={styles.subMessage}>
            Please grant camera access to scan company QR codes
          </Text>
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.title}>Scan Company QR Code</Text>
